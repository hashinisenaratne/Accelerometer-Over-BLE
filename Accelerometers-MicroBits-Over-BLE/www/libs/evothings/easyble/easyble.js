...
;(function()
{
	...
	/**
	 * Disconnect and close a given connected BLE device.
	 * @example
	 *   evothings.easyble.closeConnectedDevice();
	 */
	evothings.easyble.closeConnectedDevice = function(device)
	{
		for (var key in internal.connectedDevices)
		{
			if(device != null && internal.connectedDevices[key] != null){
				if(device.address == internal.connectedDevices[key].address){
					device && device.close();
					internal.connectedDevices[key] = null;
				}
			}
	 ...
	 * @example
	 *   evothings.easyble.getConnectedDevices();
	 */
	evothings.easyble.getConnectedDevices = function()
	{
		var devices = internal.connectedDevices;
		return devices;
	};
